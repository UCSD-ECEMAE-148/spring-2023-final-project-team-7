...
class PathPlanner(Node):
    def __init__(self):
        ...
        self.detection_subscriber = self.create_subscription(Float32, DETECTION_TOPIC_NAME, self.detector, 10)
    ...
    def detector(self, floatmsg):
        
        # Set throttle to 0 when detecting person/car

        if floatmsg.data < 0.4:
             self.detectionflag = True
             self.twist_cmd.linear.x = -0.5
             self.twist_publisher.publish(self.twist_cmd)
             self.starttime = time.time()

        elif floatmsg.data < 0.7:
            self.detectionflag = True
        ...
        elif time.time()-self.starttime > 1.7:
            self.detectionflag = False
        
        print(self.detectionflag)
